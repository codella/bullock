...
describe Bullock::Parse::Production do
  ...
  describe ".new" do
    it "creates production with a single expansion symbol" do
      production = Bullock::Parse::Production.new(:symbol, 'a', &void)

      expect(production.expansion.length).to eq 1
    end

    it "creates production with two expansion symbols" do
      production = Bullock::Parse::Production.new(:symbol, 'a b', &void)
  ...
  describe "#==" do
    it "returns false when symbol is different" do
      production = Bullock::Parse::Production.new(:symbol, 'expansion', &void)
      other_production = Bullock::Parse::Production.new(:another_symbol, 'expansion', &void)
    ...
    it "returns false when expansion is different" do
      production = Bullock::Parse::Production.new(:symbol, 'expansion', &void)
      other_production = Bullock::Parse::Production.new(:symbol, 'another_expansion', &void)
    ...
    it "returns true when symbol and expansion are == other production's" do
      production = Bullock::Parse::Production.new(:symbol, 'expansion', &void)
      other_production = Bullock::Parse::Production.new(:symbol, 'expansion', &void)
